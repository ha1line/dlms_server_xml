...
namespace dlms_cosem_xml
{

	namespace bpt = boost::property_tree;

	const char uspd_ini_path[] = "/home/putin/Downloads/uspd.ini";
	...
	INIParser::INIParser(const char* uspd_ini_path)
	{
		...
		try
		{
			if (in.is_open())
			{
				...
				for (bpt::iptree::const_iterator it = m_iniTree.begin(); it != m_iniTree.end();
				     ++it)
				{
					if (it->first.find("Module") != std::string::npos)
					{
						...
						if (moduleType.find("DM") != std::string::npos ||
						    ...
						    moduleType.find("P") != std::string::npos ||
						    moduleType.find("M") != std::string::npos)
						{
	...
	const bpt::iptree XMLParser::MakePTree() const
	{
		...
		try
		{
			bpt::iptree& defaultServerTree(
				xmlTree.add_child(DlmsCosemXmlNames::SERVER, bpt::iptree()));
